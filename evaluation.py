...
def write_metrics_csv(
    ...
    """Write one held-out reconstruction row per target."""
    rows: list[dict[str, object]] = []
    for target in range(target_source_ids.size):
        for method, name in enumerate(METHOD_NAMES):
            rows.append({
                ...
                "lambda": ";".join(f"{value:.9f}" for value in shared_lambdas[target, method]),
                "lambda_fit_rmse": f"{lambda_fit_rmse[target]:.9e}",
                **{
