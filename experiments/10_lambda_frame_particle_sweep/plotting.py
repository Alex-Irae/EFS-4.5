...
def plot_two_pass_target(path: Path, arrays: dict[str, np.ndarray]) -> None:
    """Compare all four lambda methods through both frames and replay."""
    ...
    direct_candidates = project(arrays["direct_candidates"][0])  # [M,P,D] -> [M,P,2]
    ...
    candidate_reference = project(arrays["reference_candidates"])  # [M,P,D] -> [M,P,2]
    ...
    candidate_generation = project(arrays["generation_candidates"][0])  # [M,P,D] -> [M,P,2]
    replay = project(arrays["target_trajectory"][:, 0])  # [J+1,M,P,D] -> [J+1,M,P,2]
    output = project(arrays["generated"][0])  # [M,P,D] -> [M,P,2]

    particle_count = target_initial.shape[0]
    method_names = arrays["method_names"].astype(str).tolist()
    method_count = len(method_names)
    colors = plt.get_cmap("tab10")(np.linspace(0.0, 0.75, method_count))
    markers = ("o", "s", "^", "D")
    ...
    axes[0, 0].scatter(target_initial[:, 0], target_initial[:, 1], marker="X", s=58, color="black", label="target")
    for method, name in enumerate(method_names):
        axes[0, 0].scatter(
            direct_candidates[method, :, 0],
            direct_candidates[method, :, 1],
    ...
    axes[0, 0].set_title("1. d1 fitted and cross-frame candidates")
    ...
    axes[0, 1].scatter(target_terminal[:, 0], target_terminal[:, 1], marker="X", s=58, color="black", label="target endpoint")
    for method, name in enumerate(method_names):
        axes[0, 1].scatter(
            candidate_reference[method, :, 0],
            candidate_reference[method, :, 1],
            marker=markers[method],
            s=30,
            facecolors="none",
            edgecolors=colors[method],
    ...
    axes[0, 1].set_title("2. Same lambdas at pass-1 du")
    axes[0, 1].legend(fontsize=8)
    ...
    axes[1, 0].scatter(target_terminal[:, 0], target_terminal[:, 1], marker="X", s=58, color="black", label="stored target endpoint")
    for method, name in enumerate(method_names):
        axes[1, 0].scatter(
            candidate_generation[method, :, 0],
            candidate_generation[method, :, 1],
            marker=markers[method],
            s=30,
            facecolors="none",
            edgecolors=colors[method],
    ...
    axes[1, 0].set_title("3. Pass-2 seeds after target removal")
    axes[1, 0].legend(fontsize=8)

    stride = max(1, (replay.shape[0] - 1) // 500)
    replay_terminal_to_initial = replay[::-1][::stride]  # shape: [F,M,P,2]
    axes[1, 1].scatter(target_initial[:, 0], target_initial[:, 1], marker="X", s=58, color="black", label="real target d1")
    for method, name in enumerate(method_names):
        for particle in range(particle_count):
            axes[1, 1].plot(
                replay_terminal_to_initial[:, method, particle, 0],
                replay_terminal_to_initial[:, method, particle, 1],
                color=colors[method],
                alpha=0.35,
                linewidth=0.8,
            )
    ...
    axes[1, 1].set_title("4. Backward arrivals versus target")
    axes[1, 1].legend(fontsize=8)
    ...
    for axis in axes.flat:
        ...
        if raw_2d:
            axis.set_aspect("equal", adjustable="box")
    figure.suptitle("Experiment 10 lambda-frame comparison")
    figure.savefig(path, dpi=180, bbox_inches="tight")
    plt.close(figure)
...
def plot_operator_inference(path: Path, arrays: dict[str, np.ndarray]) -> None:
    """Show operator parents, the terminal seed, and replay self-consistency."""
    dimension = arrays["initial_plane"].shape[1]
    if dimension == 2:
        mean = np.zeros((1, 2))
        components = np.eye(2)
        labels = ("coordinate 1", "coordinate 2")
        raw_2d = True
    else:
        mean, components, ratio = _fit_pca(
            arrays["initial_plane"],
            arrays["history"][-1],
            arrays["direct_candidate"],
            arrays["generated"],
        )
        labels = (f"PC1 ({100 * ratio[0]:.1f}%)", f"PC2 ({100 * ratio[1]:.1f}%)")
        raw_2d = False

    def project(values: np.ndarray) -> np.ndarray:
        return (values - mean) @ components.T  # [...,D] @ [D,2] -> [...,2]

    initial = project(arrays["initial_plane"])
    terminal = project(arrays["history"][-1])
    parents_d1 = project(arrays["matched_initial_parents"])
    parents_du = project(arrays["matched_terminal_parents"])
    direct = project(arrays["direct_candidate"])
    seed = project(arrays["terminal_candidate"])
    trajectory = project(arrays["operator_trajectory"][:, 0])  # [J+1,P,D] -> [J+1,P,2]
    generated = project(arrays["generated"])

    figure, axes = plt.subplots(1, 3, figsize=(15.5, 5.0), constrained_layout=True)
    axes[0].scatter(initial[:, 0], initial[:, 1], s=4, color="#BBBBBB", alpha=0.18)
    axes[0].scatter(parents_d1[:, :, 0], parents_d1[:, :, 1], s=20, color="#377EB8", alpha=0.6, label="parents")
    axes[0].scatter(direct[:, 0], direct[:, 1], marker="X", s=48, color="#E41A1C", label="direct d1 mixture")
    axes[0].set_title("1. Operator parents and direct mixture")

    axes[1].scatter(terminal[:, 0], terminal[:, 1], s=4, color="#BBBBBB", alpha=0.18)
    axes[1].scatter(parents_du[:, :, 0], parents_du[:, :, 1], s=20, color="#377EB8", alpha=0.6, label="same parents")
    axes[1].scatter(seed[:, 0], seed[:, 1], marker="X", s=48, color="#984EA3", label="terminal seed")
    axes[1].set_title("2. Same lambda at du")

    stride = max(1, (trajectory.shape[0] - 1) // 500)
    terminal_to_initial = trajectory[::-1][::stride]  # shape: [F,P,2]
    for particle in range(terminal_to_initial.shape[1]):
        axes[2].plot(
            terminal_to_initial[:, particle, 0],
            terminal_to_initial[:, particle, 1],
            color="#4DAF4A",
            alpha=0.6,
            linewidth=0.9,
        )
    axes[2].scatter(direct[:, 0], direct[:, 1], marker="X", s=48, color="#E41A1C", label="direct d1 mixture")
    axes[2].scatter(generated[:, 0], generated[:, 1], marker="o", s=38, facecolors="none", edgecolors="#4DAF4A", label="replayed output")
    axes[2].set_title("3. Replay versus direct d1 mixture")
...
def plot_all(run_directory: Path) -> list[Path]:
    ...
    written: list[Path] = []
    if "operator_protocol" in arrays:
        if "generated" in arrays:
            path = run_directory / "operator_inference.png"
            plot_operator_inference(path, arrays)
            written.append(path)
        return written
