...
- `../../requirements.txt`: the repository-wide dependency file.
...
Raw NPZ and NPY files are generated locally but ignored by Git. CSV, JSON, summaries, and figures are sufficient to inspect and regenerate the published plots.

## Deferred joint-vector control

The original plan also proposed flattening each small source from `[P,D]` to one `[P*D]` particle and comparing that joint EFS map with passive particle-wise replay. It remains deferred because it introduces a different high-dimensional EFS model. It should only be revived if the three implemented diagnostics leave passive H4.5 scientifically viable.
