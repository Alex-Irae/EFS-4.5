...
- `../../requirements.txt`: repository-wide NumPy and Matplotlib versions.
...
```powershell
python run.py --particles 256 --forward-steps 400 --pairs 3 --betas 0.05 0.1 0.2 --workers 3 --output-root results\smoke_check
```

Add an 11D field after the smaller runs are interpretable:

```powershell
python run.py --dimensions 2 4 11 --particles 768 --workers 5 --output-root results\11d
```
...
Raw NPZ files are generated locally but ignored by Git. The repository keeps CSV, JSON, summaries, and figures as the publishable evidence.
