...
The canonical implementation files remain at the repository root because experiments 04 through 07 share them. Run the commands below from that root.
...
- `../05_point_interpolation/config.json`: single-particle uniform control parameters.
- `../06_strong_contraction/config.json`: long, strongly contracting 2D parameters.
- `../../requirements.txt`: repository-wide NumPy and Matplotlib requirements.
- `results/`: two-pass result directories owned by this experiment.
- `../`: chronological folders containing the other preserved experiments.
...
```powershell
python search.py --resume experiments\03_slot_single_calibration\results\<search-directory> --workers 4
```

Small path check that never replaces the active config:

```powershell
python search.py --quick --workers 4 --output-root experiments\03_slot_single_calibration\results
```
...
```powershell
$workers = [Math]::Min(32, [Environment]::ProcessorCount)
python run.py --protocol single-pass-toy --config experiments\05_point_interpolation\config.json --distribution uniform --dimension 2 --particles-per-source 1 --source-count 4096 --heldout-sources 64 --parents 4 --vertex-sources 16 --workers $workers --log-every 50 --seed 501 --output-root experiments\05_point_interpolation\results
```
...
```powershell
$workers = [Math]::Min(32, [Environment]::ProcessorCount)
python run.py --protocol single-pass-toy --config experiments\06_strong_contraction\config.json --distribution chaotic --dimension 2 --particles-per-source 8 --source-count 512 --heldout-sources 16 --parents 4 --vertex-sources 16 --workers $workers --log-every 50 --seed 502 --output-root experiments\06_strong_contraction\results
```
...
```powershell
python run.py --quick --output-root experiments\07_two_pass_reconstruction\results
```

Explicitly override scale while retaining other saved values:

```powershell
python run.py --source-count 768 --particles-per-source 10 --output-root experiments\07_two_pass_reconstruction\results
```

Ignore the saved calibration:

```powershell
python run.py --ignore-config --output-root experiments\07_two_pass_reconstruction\results
```
...
This requires the local `samples.npz` created by that run. Generated NPZ files are ignored by Git, so the committed historical result keeps its figures and readable metrics but cannot be replotted without recreating the raw array locally.
